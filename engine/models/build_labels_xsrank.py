...
## 2026-09-03 訓練結果：兩個結構性問題解決了，績效反而更差（見 doc/BACKTEST_LOG.md #32）

正例率跨切分完全穩定（五個切分全部 0.200），分數上限勉強摸到滑桿下限（0.5010，
1,035,117 列中只有 1 列 ≥0.50）。**但這只解決了「挑不挑得到門檻」，沒解決
「模型有沒有鑑別力」**：val_sel AUC 0.5478，比 `label_steady20` 的 0.5541 還差。

**最重要的教訓**：label 側全歷史分析算出的崩跌/持平偏差是 1.12（幾乎打平），
但模型真正選出來的高分訊號（每日前 1.5%）崩跌股佔比仍有 18.1%（`label_up20`
是 18.9%，全市場基準只有 11.1%）——**幾乎沒有改善**。原因：label 分析用的是
全歷史全樣本統計，模型的訊號是被 RF 揀選過的極端 1.5%，兩個群體不是同一回事。
`label_up20` 的「數天數」設計雖有崩跌偏差，卻讓模型學出穩定訊號（Sharpe 0.777）；
兩輪改法（`label_steady20`、`label_xsrank20`）都讓 AUC 掉到接近隨機、
Sharpe 腰斬以上。**目標函數的「可學習性」與「無偏性」在這個問題上有張力，
不是免費的午餐。**

**⚠️ 往後任何新 label 都要先用小樣本快速驗證「真正訓練出的模型」的高分訊號組成，
不能只看 label 本身的全歷史統計就判斷方向對不對** —— 這是本輪最貴的教訓，
兩輪都在訓練完才發現。
