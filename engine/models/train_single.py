...
ROUND_SPLITS: dict[int, dict[str, tuple[str, str]]] = {
    ...
    4: {
        # 2026-08-25：val_es 與 val_sel 之間補上一個月 embargo（2024-06）。
        # 舊版兩者相接，val_es 末端的 label_up20 視窗（未來 20 個交易日）會伸進
        # val_sel 的前 20 天 —— 調參集與挑門檻集有重疊，門檻選擇被輕微污染。
        # 兩者都是驗證集所以不影響 train 的乾淨度，但既然 train↔val 與 val↔test
        # 都留了 embargo，這裡沒有理由不留。
        "train": ("2020-01-01", "2023-11-30"),
        "val_es": ("2024-01-01", "2024-05-31"),    # 調參目標（embargo 2024-06）
