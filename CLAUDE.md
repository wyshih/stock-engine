...
| m1_steady20 | base | 344 | label_steady20 | ⛔ 不出貨 |
| m1_xsrank20 | base | 344 | label_xsrank20 | ⛔ 不出貨 |

⚠️ **`m1_steady20` / `m1_xsrank20` 是實驗模型，不會畢業（2026-09-03 結論）**：
兩輪都試圖修 `label_up20` 的崩跌反彈偏差，label 側都成功（偏差從 1.78 分別壓到
0.98 / 1.12），但**訓練出的模型績效都比 label_up20 差很多**——
val_sel AUC 接近隨機（0.554 / 0.548）、Sharpe 從 0.777 掉到 0.445 / 0.264。
**最貴的教訓**：label 側全歷史統計算出的偏差不代表模型真正選出的高分訊號
（被 RF 揀選過的極端 1.5%）也不偏頗 —— 兩者是不同的群體，兩輪都是訓練完才
發現這件事。完整記錄見 `doc/BACKTEST_LOG.md` #31 / #32 與
`build_labels_steady.py` / `build_labels_xsrank.py` 的模組 docstring。

兩者都列在 `bundle.EXPERIMENTAL_KEYS`：沒有門檻、不在 `summary.MODEL_KEYS`、
不進回測表與公開資料包。**建議暫停「修 label_up20 崩跌偏差」這個方向** ——
下次要做，先用小樣本訓練驗證高分訊號的組成，再決定值不值得整輪訓練。

**四者用同一份特徵集、同一個搜尋空間，只差標的** —— 差異只能來自標的，
不會混進調參的運氣。`m1_steady20` / `m1_xsrank20` 這一輪**都沒有調參**
（借 `m1_base_up20` 的組態，bundle 的 `config_source_key` 有留痕）——
成效不好，沒有走到規定的各自調參那一步。

- `label_mdd10` 是 `label_up20` 再要求「未來 20 個交易日內最低收盤不跌破 −10%」，
  原本標 1 但期間跌破的樣本改標 **0**（不是整列排除）。
- `label_steady20` 是**獨立定義**：未來 20 日報酬 >
  `max(1.5 × 進場前20日波動 × √20, 5%)`，且未來 20 日至少 10 天收盤站上 20 日線。
- `label_xsrank20` 也是**獨立定義**：(未來20日報酬/進場前20日波動) 在**當日
  全市場**的百分位進前 20%。
...
**97.8% 是過去 20 日跌超過 10% 的股票**（全市場只有 13.5%），正例裡「盤整緩漲」
只佔 53.3%、比全市場基準 56.7% 還低。`label_steady20` 就是為了修這件事，
把偏差壓到 0.98。完整的實驗記錄與**試過但無效的五個方向**寫在
`engine/models/build_labels_steady.py` 的模組 docstring 裡，改標的之前先讀。
