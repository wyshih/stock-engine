...
## 第 3 個模型：swing（2026-09-05 新增，前端可用，**與 m1 完全獨立**）

| 代號 | 特徵集 | 特徵數 | label | 模型 | 門檻 |
|---|---|---|---|---|---|
| swing | SWING（技術＋籌碼，不含 mkt_* 與基本面） | 316 | `label_swing_up` | LightGBM | 買 0.97 / 賣 0.20 |

`label_swing_up` 來自 `build_labels_swing.py` 的**波段九分類**：ZigZag 5% 切上漲/
下跌腿，Kaufman 效率比（置中 21 日）< 0.15 覆寫成盤整，連續同狀態區間按時間切成
起/中/末。二元標的只留「上漲-起 = 1、下跌-起 = 0」，其餘七類 NA 不進訓練。
全市場 1,835/2,071 檔、278 萬筆標籤、72 萬筆可訓練，正例率 0.502。
AUC：val_es 0.769 / val_sel 0.748 / test 0.729 / test2 0.713。

**⚠️ 出場規則跟 m1 不同**：不是 `CURRENT_EXIT_RULES`，是「**分數跌回 0.20 以下
就賣**」，走 `engine/backtest/score_exit.py`（另開一支，刻意不在 `simulate()` 裡
加分支——那是 m1 兩個模型共用的唯一路徑）。bundle 的 `exit_rule` 欄位記著這件事，
`bundle.exit_rule()` 用 `.get()` 取，舊 bundle 沒有這一欄就是既有行為。
streamlit 的回測頁靠 `_run_backtest()` 分流，並會提示「移動停利設定對它不生效」。

**回測（買 0.97 賣 0.20，dedup=False，成本 0.585%，日成交額 >= 2000 萬）**：
505 筆、每筆平均 +15.01%、中位 +12.62%、勝率 77.2%、平均抱 47 天。
扣掉**同一天進出場的等權大盤**後 +7.28%，三段裡贏兩段：

| 期間 | 扣掉大盤後 | 說明 |
|---|---|---|
| 2024H2 | +1.76% | 大盤 -4.6%、最大回檔 -14.4% |
| 2025H2 | **-2.22%** | 大盤 +9.0%、最大回檔只有 -6.1% |
| 2026H1 | +10.91% | 大盤 +7.5%、最大回檔 -15.3% |

**⚠️ 這是逆勢型模型。** 抓的是「跌完之後的轉折」，沒有大跌就沒有舞台：2025H2
高分訊號只有 37 個（訊號密度 0.04%，另兩期是 0.14% / 0.26%），而且 51.4% 落在
「下跌-末」而不是「上漲-起」。逐月看，大盤跌的月份贏、大盤漲的月份輸。
**訊號數突然變少本身就是「現在不該用它」的信號**，當天就看得到。

**走過的四條死路（常數留在程式碼裡註明「不要再試」）**：
1. `MIN_RUN_DAYS=10`（合併碎片段落）→ AUC 0.75→0.87，但「起」從 1 天拉成 12 天，
   模型改讀已發生的動能，高分股從「剛跌下來」變成「已經漲很多」，回測全面變差。
2. 特徵加基本面（`revenue_*`/`per`/`pbr`/`dividend_yield`）→ 重要度前五名全被佔據，
   變成價值＋動能。
3. `MIN_MOVE=0.10`（把「值得交易」寫進 ground truth）→ 抓到起漲的倍數 2.80x→1.55x、
   好壞比 11.5→4.99。**「會往哪邊走」和「會走多大」是兩種難度。**
4. 訓練用「全部上漲 vs 全部下跌」（只排除平盤）→ 資料多 2.4 倍、AUC 0.75→0.80，
   但中位報酬從 +12.62% 掉到 +0.65%。**中段末段必須排除。**

**共通教訓：這四次每一次 AUC 都上升、實際報酬都下降。** AUC 量的是「認得出現在的
狀態」，錢來自「認得出還沒發生的轉折」，在這個問題上兩者反向。

⚠️ **embargo 比 m1 長**（`train_swing.SPLITS`）：train→val_es 六個月、
val_es→val_sel 兩個月、val_sel→test 六個月。實測標籤解析延遲中位數 21 個交易日、P90 57。

⚠️ 分數檔（`score_swing_{split}` 與 `score_live_swing`）一律**全市場口徑**。只對
有 label 的列打分是事後口徑，餵進回測母體會少 85%。`--no-live` 時直接 raise。

⚠️ **前處理走 `train_single.fit_stats/apply_stats`**（中位數補值＋標準化＋winsorize）。
LightGBM 本身吃得下 NaN，但推論端 `bundle.score_single()` 一律會套 `apply_stats`，
訓練不套的話前端算出來的分數會對不上而且不報錯。已驗證兩條路徑逐列相同。

⚠️ **還原權值是必要步驟**，缺 `exright.parquet` 直接 raise。已知缺口：exright
**只涵蓋上市（TWSE），不含上櫃（TPEX）**（`validate_data.py:139`），上櫃股配息日
會被當成小幅下跌。

**尚未做**：`doc/BACKTEST_LOG.md` 條目、`make train` 沒接 `train_swing`、
關注清單的出場狀態（`_exit_status`）還是走移動停損那套、只有 3 個半年期的驗證。
